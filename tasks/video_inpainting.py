...
class VideoInpaintingGuidance:
    ...
    def prepare_target(self, video_path, device):
        ...
        frames = transform((frames/255.).float()).to(device)#.float()
    ...
    def get_guidance(self, x_need_grad, func=lambda x: x, post_process=lambda x:x, return_logp=False, check_grad=True, **kwargs):
        ...
        print(">>>> X: " , x.shape) ## torch.Size([8, 3, 256, 256])
...
if __name__ == "__main__":
    args = None
    guide = VideoInpaintingGuidance(args)

    vide = guide.prepare_target_video('datasets/vggsound/reference_videos_2secs/aAkFRt2rZIg_000130.mp4')
