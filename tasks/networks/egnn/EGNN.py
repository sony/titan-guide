...
import numpy as np
import math
from .utils import remove_mean_with_mask, remove_mean
...
class EGNN_energy_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
        ...
        if mode == 'egnn_dynamics':
            from .energy import EGNN as energy_used_EGNN
            self.egnn = energy_used_EGNN(
                in_node_nf=in_node_nf + context_node_nf, in_edge_nf=1,
                hidden_nf=hidden_nf, device=device, act_fn=act_fn,
                n_layers=n_layers, attention=attention, tanh=tanh, norm_constant=norm_constant,
                inv_sublayers=inv_sublayers, sin_embedding=sin_embedding,
    ...
    def _forward(self, xh, t, node_mask, edge_mask):
        bs, n_nodes, dims = xh.shape
        if t is None:
            t = torch.zeros(bs, 1).to(xh)
