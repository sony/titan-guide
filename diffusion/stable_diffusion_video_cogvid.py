...
class StableDiffusionVideoCogVidSampler(BaseSampler):
    ...
    def __init__(self, args: Arguments):
        ...
        self.weight_type = torch.float32 if "titan" in self.args.guidance_name else torch.float16
    ...
    def _build_diffusion(self, args):
        ...
        '''
        weight_type = self.weight_type #torch.float16
        model_name = "/mnt/data2/chris/code/SDS-Bridge/2D_experiments/models/Diffusion_Transformer/CogVideoX-Fun-V1.1-2b-InP"
        vae = AutoencoderKLCogVideoX.from_pretrained(
            model_name, 
            subfolder="vae"
        ).to(weight_type).to(self.device)

        text_encoder = T5EncoderModel.from_pretrained(
            model_name, subfolder="text_encoder", torch_dtype=weight_type
        ).to(self.device)
    ...
    def sample(self, sample_size: int, guidance: BaseGuidance):
        ...
        for batch_id in range(n_batchs):
            ...
            print(">>> latents sd cogvid.,::: ", latents.shape, self.height, self.width ,latents.dtype) ##    >> should be torch.randn( [1, 1, 16, 32, 48]
...
#         # prepare unet, prev_t, alpha_prod, alpha_prod_prev...
#         self.save_path = args.save_path
#         self.weight_type = torch.float32 if "fwd" in self.args.guidance_name else torch.float16
#         self.audio_path='/mnt/data2/dataset/vggsound/svg_no/audio2video/audio'
#         self._build_diffusion(args)
...
#                 # export_to_gif(videos[0], f"logs/ims_fwdvideo/animation_.gif")
#                 # export_to_video(videos[0], f"/mnt/data2/chris/outputs/vgg/test_seeinghearing/{self.text_id}.mp4", fps=8)
#                 # print("SAVE path >>>> ", f"{self.save_path}/{self.text_id}_iter{str(t)}_nostyle.mp4")
#                 # export_to_video(videos[0], f"{self.save_path}/{self.text_id}_iter{str(t)}_nostyle.mp4", fps=8)
