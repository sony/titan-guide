...
class StableDiffusionVideoCogVidSampler(BaseSampler):
    ...
    def _build_diffusion(self, args):
        ...
        # transformer.train()
        # transformer._set_gradient_checkpointing(module=None, value=True)
    ...
    def _prepare_prompts(self, args):
        ...
        elif 'parti_prompts' in args.dataset :
            prompts = [line.strip() for line in open(args.dataset, 'r').readlines()]#[:args.num_samples]
        else:
    ...
    def sample(self, sample_size: int, guidance: BaseGuidance):
        ...
        for batch_id in range(n_batchs):
            ...
            if self.args.task == "imagebind_guidance":
                guidance.guider.prepare_target(target, device=self.device)
 
            latents = torch.randn( [1, 8, 16, 32, 48], #[1, 8, 16, 32, 48],
            ...
            # inpaint_latents_no = torch.cat([mask_latents, masked_video_latents], dim=2).to(latents.dtype)
            ...
            for t in tqdm(range(1, self.inference_steps), total=self.inference_steps):
                ...
                def stable_diffusion_unet(latents, t):
                    ...
                    noise_pred = self.unet(#latent_model_input, t[0], 
                                ...
                                return_dict=False,
                                inpaint_latents=inpaint_latents[:, :latent_model_input.shape[1]])[0]

                    # print(">> inpaint_latents, noise_pred: ", inpaint_latents.shape, noise_pred.shape) ## torch.Size([2, 2, 17, 32, 48]) torch.Size([2, 2, 16, 32, 48])
                    # print(">>>>>>>>>>>>>> STABLE DIFF 1 input")
                    # with torch.no_grad():
                    #     noise_pred_uncond = self.unet(#latent_model_input, t[0], 
                    #                 hidden_states=latent_model_input[:1],#.float(),
                    #                 encoder_hidden_states=prompt_embeds[:1],#.float(),
                    #                 timestep=timestep[:1],#,.float(),
                    #                 image_rotary_emb=None,
                    #                 return_dict=False,
                    #                 inpaint_latents=inpaint_latents[:1])[0]
                    
                    # noise_pred_text = self.unet(#latent_model_input, t[0], 
                    #             hidden_states=latent_model_input[1:2],#.float(),
                    #             encoder_hidden_states=prompt_embeds[1:2],#.float(),
                    #             timestep=timestep[1:2],#,.float(),
                    #             image_rotary_emb=None,
                    #             return_dict=False,
                    #             inpaint_latents=inpaint_latents[1:2])[0]

                    # noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=prompt_embeds)[0]

                    # print(">> noise_pred_uncond, noise_pred_text: ", noise_pred_uncond.shape, noise_pred_text.shape) ## torch.Size([1, 2, 32, 32, 48]) torch.Size([2, 2, 16, 32, 48])
                    # perform guidance
                    if self.do_classifier_free_guidance:
