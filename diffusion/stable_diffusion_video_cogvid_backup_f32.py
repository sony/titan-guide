...
class StableDiffusionVideoCogVidSampler(BaseSampler):
    ...
    def _build_diffusion(self, args):
        ...
        # sample_size = [256, 384]#[384, 672]#[256, 424]#[256, 384]
        # video_length = 49
        # input_video = torch.zeros([1, 3, video_length, sample_size[0], sample_size[1]])
        # input_video_mask = torch.ones([1, 1, video_length, sample_size[0], sample_size[1]]) * 255
        # frames = self.sd_pipeline("dog barking under water, high resolution", 
        ...
        #                     video        = input_video,
        #                     mask_video   = input_video_mask,).videos #[0]

        # print(">> frames: ", frames.shape) ## torch.Size([1, 3, 49, 384, 672]) >> should be torch.Size([1, 3, 48, 256, 384])
        # videos = self.video_processor.postprocess_video(video=frames, output_type="pil")
        # videos = videos[0]
        # # print(">> frames: ", frames.shape)
        # export_to_video(videos, "logs/debug_pipeline/generated_test.mp4", fps=7)
    ...
    def sample(self, sample_size: int, guidance: BaseGuidance):
        ...
        for batch_id in range(n_batchs):
            ...
            #     generator=self.generator
            # )[0]
            latents = torch.randn( [1, 2, 16, 32, 48], #[1, 8, 16, 32, 48],
                                generator=self.generator,
                                # device=,
                ...
                def stable_diffusion_unet(latents, t):
                    ...
                    with torch.no_grad():
                        ...
                        noise_pred = self.unet(#latent_model_input, t[0], 
                                    ...
                                    return_dict=False,
                                    inpaint_latents=inpaint_latents.float())[0]
