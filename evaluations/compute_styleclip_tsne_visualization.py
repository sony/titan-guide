...
def compute_style_directly(video_paths ,target='/mnt/data2/chris/code/Training-Free-Guidance/data/wikiart/0.png'):
	...
	for video_path in video_paths:
		...
		# print(">> embed.mean(1).squeeze().unsqueeze(0): ", embed.mean(0).mean(1).squeeze().unsqueeze(0).shape)
		# image_embeds.append(embed.mean(1).mean(0).squeeze().unsqueeze(0))
		image_embeds.append(embed[:num_samples].mean(1))
	...
	image_embeds.append(target_embed.mean(1).squeeze().unsqueeze(0))
	...
	for idx in range(1, 20):
		# 各ラベルのデータを取得
		# indices = y_subset == 
		indices = [i for i, value in enumerate(video_paths) if f'iter{idx}.mp4' in value]
		print(">>> indices: ", indices)
		iidx = indices[0]
		# plt.scatter(X_tsne[indices[0], 0], X_tsne[indices, 1], label=str(idx), alpha=0.7, color=cm.Purples(idx/20))
		ax.scatter(X_tsne[iidx*num_samples:(iidx+1)*num_samples, 0], X_tsne[iidx*num_samples:(iidx+1)*num_samples, 1], label=str(idx), alpha=0.99, color=cm.Purples(idx/20), s=205)
		ax.annotate(str(idx), (X_tsne[iidx*num_samples:(iidx+1)*num_samples, 0]+offset, X_tsne[iidx*num_samples:(iidx+1)*num_samples, 1]-0.25))


	for idx in range(1, 20):
		# 各ラベルのデータを取得
		# indices = y_subset == 
		indices = [i for i, value in enumerate(video_paths) if f'iter{idx}_nostyle.mp4' in value]
		print(">>> indices: ", indices)
		iidx = indices[0]
		# plt.scatter(X_tsne[indices[0], 0], X_tsne[indices, 1], label=str(idx), alpha=0.7, color=cm.Purples(idx/20))
		ax.scatter(X_tsne[iidx*num_samples:(iidx+1)*num_samples, 0], X_tsne[iidx*num_samples:(iidx+1)*num_samples, 1], label=str(idx), alpha=0.99, color=cm.Blues(idx/20), s=205)
		ax.annotate(str(idx), (X_tsne[iidx*num_samples:(iidx+1)*num_samples, 0]+offset, X_tsne[iidx*num_samples:(iidx+1)*num_samples, 1]+offset))

	# plt.scatter(X_tsne[:-1, 0], X_tsne[:-1, 1], label="test", alpha=0.7)
	ax.scatter(X_tsne[-1, 0], X_tsne[-1, 1], label="reference", alpha=0.99, color="red", marker="*", s=305, )
	ax.annotate("Target", (X_tsne[-1, 0]+offset, X_tsne[-1, 1]+offset))
	
	
	# plt.scatter(X_tsne[:, 0], X_tsne[:, 1], label="test", alpha=0.7)
	
	# plt.legend()
	plt.title("t-SNE Diffusion Trajectory Visualization")
	# plt.xlabel("t-SNE Dimension 1")
	# plt.ylabel("t-SNE Dimension 2")
	plt.grid(True)
	plt.tight_layout()
	plt.savefig("../logs/tsne.pdf")
...
if __name__== "__main__":
	...
	generated_video_root2 = "/mnt/data2/chris/outputs/video_gen_guide/style_transfer/titan_eps_cogvid_tsne_ablation/"
	gt_video_paths = glob.glob(f"{gt_video_root}/*.mp4")
	generated_video_paths = glob.glob(f"{generated_video_root}/*.mp4")
	generated_video_paths2 = glob.glob(f"{generated_video_root2}/*.mp4")
	image_size = (256, 256)
	video_fps= 8
	video_num_frame = 16

	generated_video_paths = generated_video_paths+generated_video_paths2
