...
class StyleCLIP(torch.nn.Module):
    ...
    @torch.no_grad()
    def get_target_embedding_feature(self, target):
        print("?? target: ", target)
        img = Image.open(target).convert('RGB')
        image = img.resize(self.image_size, Image.Resampling.BILINEAR)
        image = self.transforms(ToTensor()(image)).unsqueeze(0)
        return self.get_embedding_feature(image)

    def get_gram_matrix(self, img):
        img = img.to(self.device)
        # print(">>> img: ", img.shape) # torch.Size([60, 256, 256])
    ...
    @torch.no_grad()
    def get_embedding_feature(self, img):
        img = img.to(self.device)
        # print(">>> img: ", img.shape) # torch.Size([60, 256, 256])
        img = torch.nn.functional.interpolate(img, size=self.image_size, mode='bicubic')
        img = self.transforms(img)
    ...
    def forward(self, x):
        ...
        # print(">> : embed: ", embed.shape)  ## embed:  torch.Size([16, 768, 768])
